    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          View user details, manage account status, and assign roles.
        </p>
      </div>
              ...
              <TableBody>
                {filteredUsers.map((user) => (
                  <TableRow key={user.uid}>
                    <TableCell className="font-mono text-xs">{user.uid.substring(0,8)}...</TableCell>
                    ...
                    <TableCell>
                      {user.dateCreated instanceof Timestamp 
                        ...
                        : user.dateCreated?.seconds // For older data potentially not yet a Timestamp client-side
      ...
      {selectedUser && (
       ...
       <p className="text-xs text-muted-foreground text-center">
            Reminder: User role and status changes are made directly to Firestore.
            Ensure Firestore Security Rules are in place to protect these operations.
            For enhanced security, role changes should ideally be handled by Cloud Functions using custom claims.
        </p>
    </div>
  );
}
